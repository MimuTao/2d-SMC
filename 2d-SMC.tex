...
%	by selecting parameter $\beta_{k}$ properly. 
	...
	\end{equation}
	for any $s\in\mathcal{N}_{2}$, where the matrix $K_{s}\in\mathbb{R}^{nu\times nx}$ with $nx=nh+nv$ will be determined later, and the parameter $\rho(i,j)$ is given as
	\begin{equation}
...
\begin{proof}
	Select the Lyapunov candidate as follows\\
	Theroem proof.
\end{proof}
